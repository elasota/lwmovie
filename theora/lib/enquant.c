...
int oc_quant_params_clone(th_quant_info *_dst,const th_quant_info *_src,const ogg_allocator *_alloc){
  int i;
  ...
  for(i=0;i<6;i++){
    ...
    if(pdup&&_src->qi_ranges[qti][pli].sizes==_src->qi_ranges[qtj][plj].sizes){
      ...
      int *sizes;
      sizes=(int *)ogghack_malloc(_alloc,nranges*sizeof(*sizes));
    ...
    if(pdup&&_src->qi_ranges[qti][pli].base_matrices==
     ...
     _src->qi_ranges[0][pli].base_matrices){
      ...
      base_matrices=(th_quant_base *)ogghack_malloc(_alloc,
