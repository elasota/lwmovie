...
static int oc_comment_unpack(oc_pack_buf *_opb,th_comment *_tc,const ogg_allocator *_alloc){
  long len;
  int  i;
  /*Read the vendor string.*/
  len=oc_unpack_length(_opb);
  if(len<0||len>oc_pack_bytes_left(_opb))return TH_EBADHEADER;
  _tc->vendor=ogghack_malloc(_alloc,(size_t)len+1);
  ...
  if(_tc->comments!=0) {
    _tc->comment_lengths=(int *)ogghack_malloc(_alloc,
     _tc->comments*sizeof(_tc->comment_lengths[0]));
    _tc->user_comments=(char **)ogghack_malloc(_alloc,
  ...
  for(i=0;i<_tc->comments;i++){
    ...
    _tc->user_comments[i]= ogghack_malloc(_alloc,(size_t)len+1);
...
static int oc_setup_unpack(oc_pack_buf *_opb,th_setup_info *_setup){
  int ret;
  memcpy(&_setup->allocator,&_opb->allocator,sizeof(ogg_allocator));
  /*Read the quantizer tables.*/
  ret=oc_quant_params_unpack(_opb,&_setup->qinfo,&_setup->allocator);
  if(ret<0)return ret;
  /*Read the Huffman trees.*/
  return oc_huff_trees_unpack(_opb,_setup->huff_tables,&_setup->allocator);
}

static void oc_setup_clear(th_setup_info *_setup){
  oc_quant_params_clear(&_setup->qinfo,&_setup->allocator);
  oc_huff_trees_clear(_setup->huff_tables,&_setup->allocator);
}
 ...
 th_comment *_tc,th_setup_info **_setup,ogg_packet *_op){
  ...
  switch(packtype){
    ...
    case 0x81:{
      ...
      ret=oc_comment_unpack(_opb,_tc,&_info->allocator);
    ...
    case 0x82:{
      ...
      setup=(oc_setup_info *)ogghack_calloc(&_info->allocator,1,sizeof(*setup));
      if(setup==NULL)return TH_EFAULT;
      ret=oc_setup_unpack(_opb,setup);
      if(ret<0){
        oc_setup_clear(setup);
        ogghack_free(&_info->allocator,setup);
      }
      else{
        *_setup=setup;
        ret=1;
...
int th_decode_headerin(th_info *_info,th_comment *_tc,
 th_setup_info **_setup,ogg_packet *_op){
  ...
  oc_pack_readinit(&opb,_op->packet,_op->bytes,&_info->allocator);
...
void th_setup_free(th_setup_info *_setup){
  if(_setup!=NULL){
    ...
    ogghack_free(&_setup->allocator,_setup);
  }
}
