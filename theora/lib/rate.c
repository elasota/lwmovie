...
void oc_rc_state_clear(oc_rc_state *_rc,oc_enc_ctx *_enc){
	ogghack_free(&_enc->state.allocator,_rc->frame_metrics);
}
...
void oc_enc_rc_resize(oc_enc_ctx *_enc){
  ...
  if(_enc->rc.twopass==2){
    ...
    if(cfm<buf_delay&&(_enc->rc.frame_metrics!=NULL||reset_window)){
      ...
      int               nfm;
      int               fmh;
      fm=(oc_frame_metrics *)ogghack_realloc(&_enc->state.allocator,_enc->rc.frame_metrics,
