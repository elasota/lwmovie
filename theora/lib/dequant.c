...
int oc_quant_params_unpack(oc_pack_buf *_opb,th_quant_info *_qinfo,const ogg_allocator *_alloc){
  ...
  base_mats=ogghack_malloc(_alloc,nbase_mats*sizeof(base_mats[0]));
  ...
  for(i=0;i<6;i++){
    ...
    /*Note: The caller is responsible for cleaning up any partially
       constructed qinfo.*/
    if(qi>63){
      ogghack_free(_alloc,base_mats);
      return TH_EBADHEADER;
    }
    qranges->nranges=qri;
    qranges->sizes=qrsizes=(int *)ogghack_malloc(_alloc,qri*sizeof(qrsizes[0]));
    ...
    qrbms=(th_quant_base *)ogghack_malloc(_alloc,(qri+1)*sizeof(qrbms[0]));
    if(qrbms==NULL){
      /*Note: The caller is responsible for cleaning up any partially
         constructed qinfo.*/
      ogghack_free(_alloc,base_mats);
...
void oc_quant_params_clear(th_quant_info *_qinfo,const ogg_allocator *_alloc){
  int i;
  for(i=6;i-->0;){
    int qti;
    int pli;
    ...
    ogghack_free(_alloc,(void *)_qinfo->qi_ranges[qti][pli].sizes);
    ogghack_free(_alloc,(void *)_qinfo->qi_ranges[qti][pli].base_matrices);
  }
}
