...
void th_info_init(th_info *_info,const ogg_allocator *_alloc){
  memset(_info,0,sizeof(*_info));
  _info->version_major=TH_VERSION_MAJOR;
  _info->version_minor=TH_VERSION_MINOR;
  _info->version_subminor=TH_VERSION_SUB;
  _info->keyframe_granule_shift=6;
  memcpy(&_info->allocator, _alloc, sizeof(*_alloc));
}
...
void th_comment_init(th_comment *_tc,const ogg_allocator *_alloc){
  memset(_tc,0,sizeof(*_tc));
  memcpy(&_tc->allocator, _alloc, sizeof(*_alloc));
}
...
void th_comment_add(th_comment *_tc,const char *_comment){
  ...
  int   *comment_lengths;
  int    comment_len;
  user_comments=ogghack_realloc(&_tc->allocator,_tc->user_comments,
   (_tc->comments+2)*sizeof(*_tc->user_comments));
  if(user_comments==NULL)return;
  _tc->user_comments=user_comments;
  comment_lengths=ogghack_realloc(&_tc->allocator,_tc->comment_lengths,
  ...
  user_comments[_tc->comments]=ogghack_malloc(&_tc->allocator, comment_len+1);
...
void th_comment_clear(th_comment *_tc){
  if(_tc!=NULL){
    long i;
    for(i=0;i<_tc->comments;i++)ogghack_free(&_tc->allocator,_tc->user_comments[i]);
    ogghack_free(&_tc->allocator,_tc->user_comments);
    ogghack_free(&_tc->allocator,_tc->comment_lengths);
    ogghack_free(&_tc->allocator,_tc->vendor);

	ogg_allocator allocator;
	memcpy(&allocator, &_tc->allocator, sizeof(allocator));
    memset(_tc,0,sizeof(*_tc));
	memcpy(&_tc->allocator, &allocator, sizeof(allocator));
  }
}
