...
static void oc_fdct8_b2_baseline(ogg_int16_t _y[8], const ogg_int16_t *_x) {
	int i0 = _x[0 << 3];
	int i1 = _x[1 << 3];
	int i2 = _x[2 << 3];
	int i3 = _x[3 << 3];
	...
	// odd stage 2
	int b4 = (a7 * 64 + a4 * 304) / 377;
	int b5 = (a5 * 256 + a6 * 176) / 377;
	int b6 = (a6 * 256 - a5 * 176) / 377;
	int b7 = (a4 * 64 - a7 * 304) / 377;

	// even stage 3
	int c0 = (b1 + b0) / 2;
	int c1 = (b0 - b1) / 2;
	int c2 = (b3 * 8 + b2 * 20) / 29;
	int c3 = (b2 * 8 - b3 * 20) / 29;
...
static void oc_fdct8_b2(ogg_int16_t _y[8], const ogg_int16_t *_x) {
	int i0 = _x[0 << 3];
	int i1 = _x[1 << 3];
	int i2 = _x[2 << 3];
	int i3 = _x[3 << 3];
...
/*Performs a forward 8x8 Type-II DCT transform.
  ...
  _x: The input coefficients. */
void oc_enc_fdct8x8_c_old(ogg_int16_t _y[64],const ogg_int16_t _x[64]){
...
void oc_enc_fdct8x8_c(ogg_int16_t _y[64],const ogg_int16_t _x[64]){
  ...
  /*These biases correct for some systematic error that remains in the full
     fDCT->iDCT round trip.*/
  /*Transform columns of w into rows of _y.*/
  for(in=w,out=_y,end=out+64;out<end;in++,out+=8)oc_fdct8_b2(out,in);
  /*Transform columns of _y into rows of w.*/
  for(in=_y,out=w,end=out+64;out<end;in++,out+=8)oc_fdct8_b2(out,in);
  /*Round the result back to the external working precision (which is still
     scaled by four relative to the orthogonal result).
    TODO: We should just update the external working precision.*/
  for(i=0;i<64;i++)_y[i]=w[OC_FZIG_ZAG[i]]+4>>3;
}
