...
void *oc_aligned_malloc(size_t _sz,size_t _align,const ogg_allocator *_alloc){
  unsigned char *p;
  if(_align-1>UCHAR_MAX||(_align&_align-1)||_sz>~(size_t)0-_align)return NULL;
  p=(unsigned char *)ogghack_malloc(_alloc,_sz+_align);
  if(p!=NULL){
    int offs;
...
void oc_aligned_free(void *_ptr,const ogg_allocator *_alloc){
  unsigned char *p;
  p=(unsigned char *)_ptr;
  if(p!=NULL){
    int offs;
    offs=*--p;
    ogghack_free(_alloc,p-offs);
  }
}


void **oc_malloc_2d(size_t _height,size_t _width,size_t _sz,const ogg_allocator *_alloc){
  size_t  rowsz;
  size_t  colsz;
  size_t  datsz;
  char   *ret;
  ...
  ret=(char *)ogghack_malloc(_alloc,datsz+colsz);
...
void **oc_calloc_2d(size_t _height,size_t _width,size_t _sz,const ogg_allocator *_alloc){
  size_t  colsz;
  size_t  rowsz;
  size_t  datsz;
  char   *ret;
  ...
  datsz=rowsz*_height;
  /*Alloc array and row pointers.*/
  ret=(char *)ogghack_calloc(_alloc,datsz+colsz,1);
  /*Initialize the array.*/
  if(ret!=NULL){
    size_t   i;
    void   **p;
...
void oc_free_2d(void *_ptr,const ogg_allocator *_alloc){
  ogghack_free(_alloc,_ptr);
}
