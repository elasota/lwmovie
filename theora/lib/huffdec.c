...
int oc_huff_trees_unpack(oc_pack_buf *_opb,
 ogg_int16_t *_nodes[TH_NHUFFMAN_TABLES],const ogg_allocator *_alloc){
  int i;
  for(i=0;i<TH_NHUFFMAN_TABLES;i++){
    ...
    tree=(ogg_int16_t *)ogghack_malloc(_alloc,size*sizeof(*tree));
...
int oc_huff_trees_copy(ogg_int16_t *_dst[TH_NHUFFMAN_TABLES],
 const ogg_int16_t *const _src[TH_NHUFFMAN_TABLES],const ogg_allocator *_alloc){
  int total;
  int i;
  total=0;
  for(i=0;i<TH_NHUFFMAN_TABLES;i++){
    ...
    total+=size;
    _dst[i]=(ogg_int16_t *)ogghack_malloc(_alloc,size*sizeof(*_dst[i]));
    if(_dst[i]==NULL){
      while(i-->0)ogghack_free(_alloc,_dst[i]);
      return TH_EFAULT;
    }
...
void oc_huff_trees_clear(ogg_int16_t *_nodes[TH_NHUFFMAN_TABLES],const ogg_allocator *_alloc){
  int i;
  for(i=0;i<TH_NHUFFMAN_TABLES;i++)ogghack_free(_alloc,_nodes[i]);
}
