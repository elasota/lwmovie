...
static int oc_dec_init(oc_dec_ctx *_dec,const th_info *_info,
 const th_setup_info *_setup){
  ...
  int qi;
  int ret;
  ret=oc_state_init(&_dec->state,_info,3,&_info->allocator);
  if(ret<0)return ret;
  ret=oc_huff_trees_copy(_dec->huff_tables,
   (const ogg_int16_t *const *)_setup->huff_tables,&_dec->state.allocator);
  if(ret<0){
  ...
  /*For each fragment, allocate one byte for every DCT coefficient token, plus
     ...
     one.*/
  _dec->dct_tokens=(unsigned char *)ogghack_malloc(&_info->allocator,(64+64+1)*
   _dec->state.nfrags*sizeof(_dec->dct_tokens[0]));
  if(_dec->dct_tokens==NULL){
    oc_huff_trees_clear(_dec->huff_tables,&_dec->state.allocator);
    oc_state_clear(&_dec->state);
    return TH_EFAULT;
  }
...
#if defined(HAVE_CAIRO)
  ogghack_free(&_dec->state.allocator,_dec->telemetry_frame_data);
#endif
  ogghack_free(&_dec->state.allocator,_dec->pp_frame_data);
  ogghack_free(&_dec->state.allocator,_dec->variances);
  ogghack_free(&_dec->state.allocator,_dec->dc_qis);
  ogghack_free(&_dec->state.allocator,_dec->dct_tokens);
  oc_huff_trees_clear(_dec->huff_tables,&_dec->state.allocator);
  oc_state_clear(&_dec->state);
}
...
static int oc_dec_postprocess_init(oc_dec_ctx *_dec){
  ...
  if(_dec->dc_qis==NULL){
    ...
    _dec->dc_qis=(unsigned char *)ogghack_malloc(&_dec->state.allocator,
  ...
  if(_dec->variances==NULL){
    ...
    frame_sz+=c_sz<<1;
    _dec->pp_frame_data=(unsigned char *)ogghack_malloc(&_dec->state.allocator,
     frame_sz*sizeof(_dec->pp_frame_data[0]));
    _dec->variances=(int *)ogghack_malloc(&_dec->state.allocator,
...
th_dec_ctx *th_decode_alloc(const th_info *_info,const th_setup_info *_setup){
  ...
  dec=oc_aligned_malloc(sizeof(*dec),16,&_info->allocator);
  if(dec==NULL||oc_dec_init(dec,_info,_setup)<0){
    oc_aligned_free(dec,&_info->allocator);
    return NULL;
  }
...
void th_decode_free(th_dec_ctx *_dec){
  if(_dec!=NULL){
    oc_dec_clear(_dec);
    oc_aligned_free(_dec,&_dec->state.allocator);
  }
}
...
int th_decode_packetin(th_dec_ctx *_dec,const ogg_packet *_op,
 ogg_int64_t *_granpos){
  ...
  if(_op->bytes==0){
    ...
    oc_pack_readinit(&_dec->opb,_op->packet,_op->bytes,&_dec->state.allocator);
