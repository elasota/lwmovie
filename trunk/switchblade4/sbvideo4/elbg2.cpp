...
typedef struct{
    ...
    std::vector<__int64> utility_inc;
    int *nearest_cb;
    int *points;
...
static int get_high_utility_cell(elbg_data *elbg)
{
    ...
    if (elbg->utility_inc[elbg->numCB-1] < INT_MAX) {
        r = av_random(elbg->rand_state) % (unsigned int)elbg->utility_inc[elbg->numCB-1] + 1;
    } else {
        r = av_random(elbg->rand_state);
        r = (av_random(elbg->rand_state) + (r<<32)) % elbg->utility_inc[elbg->numCB-1] + 1;
    }
