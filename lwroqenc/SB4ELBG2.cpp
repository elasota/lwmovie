...
typedef struct{
	...
	std::vector<lwmSInt64> utility_inc;
	int *nearest_cb;
	int *points;
...
static int get_high_utility_cell(elbg_data *elbg)
{
	...
	if (elbg->utility_inc[elbg->numCB - 1] < INT_MAX) {
		r = elbg->rand_state->Random() % (unsigned int)elbg->utility_inc[elbg->numCB - 1] + 1;
	}
	else {
		r = elbg->rand_state->Random();
		r = (elbg->rand_state->Random() + (r << 32)) % elbg->utility_inc[elbg->numCB - 1] + 1;
	}

	while (elbg->utility_inc[i] < static_cast<lwmSInt64>(r)) {
		i++;
	}
	...
	SB4RandomState *rand_state)
{
	...
	elbg->utility.clear();
	elbg->cells.clear();
	elbg->utility_inc.clear();
	elbg->scratchbuf.clear();
}
